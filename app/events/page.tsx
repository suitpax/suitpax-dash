...
export default function EventsPage() {
  ...
  const events = [
    ...
    },
    {
      ...
      status: "upcoming",
      isPublic: false,
      image: "/placeholder.svg?height=200&width=400&query=modern product launch event with screens",
    },
    {
      id: "event-3",
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white mb-2">Company Events</h1>
          <p className="text-white/70">Manage your company's events and gatherings</p>
        </div>
        <button
          onClick={() => setShowCreateModal(true)}
          className="mt-4 md:mt-0 flex items-center px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-white"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredEvents.map((event) => (
          <Link href={`/events/${event.id}`} key={event.id} className="block">
            <div className="bg-black border border-white/10 rounded-lg overflow-hidden hover:border-white/20 transition-all">
              <div className="relative h-48 w-full">
                ...
                {event.status === "past" && (
                  <div className="absolute top-3 left-3 bg-white/10 text-white/70 px-2 py-1 rounded text-xs">Past</div>
                )}
              </div>
              <div className="p-4">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-black border border-white/10 rounded-lg w-full max-w-2xl">
