...
import { Car, MapPin, Calendar, Users, Clock, Search, CheckCircle, PlusCircle, Edit3 } from "lucide-react"
...
interface Transfer {
  ...
  date: string
  time: string
  vehicleType: string // e.g., Sedan, SUV, Van, Luxury
  vehicleModel?: string
  passengers: number
  price: number
...
export default function TransfersPage() {
  ...
  const handleSearchTransfers = async (e: React.FormEvent) => {
    ...
    // Mock search results based on inputs - in reality, this would be an API call
    const results = mockTransfers
      ...
      .map((t) => ({
        ...
        price: t.price + (Math.random() * 50 - 25), // Randomize price a bit
      }))
      .slice(0, 3 + Math.floor(Math.random() * 3))
    setSearchResults(results)
    setLoadingSearch(false)
    setActiveTab("book") // Ensure results are shown on book tab
  }
  ...
  const transfersToShow = useMemo(() => {
    ...
    return [] // Book tab shows search results
  }, [activeTab])
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <header className="bg-white/5 border border-white/10 rounded-2xl p-6 md:p-8 backdrop-blur-sm">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            ...
            >
              <PlusCircle className="h-4 w-4 mr-2" /> Book New Transfer
            </Button>
          </div>
        </header>
        ...
        <div className="flex space-x-1">
          {(["book", "upcoming", "past"] as const).map((tab) => (
            <Button
              ...
              className={`capitalize text-sm px-3 py-1.5 h-auto rounded-full ${activeTab === tab ? "bg-white text-black hover:bg-white/90" : "border-white/20 text-white/70 hover:bg-white/10 hover:text-white"}`}
            >
              {tab === "book" ? (
            ...
            <CardContent>
              <form onSubmit={handleSearchTransfers} className="space-y-4">
                ...
                <Select value={vehiclePreference} onValueChange={setVehiclePreference}>
                  <SelectTrigger className="h-11 bg-white/5 border-white/10 text-white rounded-xl hover:bg-white/10">
                    <Car className="h-4 w-4 text-white/50 mr-2" />
                      ...
                      <div className="flex flex-col md:flex-row">
                        ...
                        <div className="p-3 flex-1">
                          <div className="flex justify-between items-start">
                            ...
                            <p className="text-lg font-semibold text-white">${transfer.price}</p>
                          </div>
              ...
              {searchResults.length === 0 && !loadingSearch && pickupLocation && (
                <div className="mt-6 text-center py-6">
                  <Car className="h-10 w-10 text-white/30 mx-auto mb-2" />
          ...
          (transfersToShow.length > 0 ? (
            ...
            <Card className="bg-white/5 border-white/10 backdrop-blur-sm">
              <CardContent className="p-10 text-center">
                <Car className="h-12 w-12 text-white/30 mx-auto mb-3" />
