...
export function MCPStatusIndicator() {
  ...
  const runMCPTest = async () => {
    setIsLoading(true)
    try {
      const response = await fetch("/api/mcp/test", { method: "POST" })
      const data = await response.json()

      // Show test results in console for debugging
      console.log("MCP Test Results:", data)
    ...
    <div className="flex items-center space-x-2">
      ...
      <div className="flex items-center space-x-1">
        <Button
          ...
          disabled={isLoading}
          size="sm"
          className="bg-transparent hover:bg-white/10 h-6 w-6 p-0 rounded-full"
      ...
      {lastCheck && <span className="text-xs text-white/50">{lastCheck.toLocaleTimeString()}</span>}

      {status?.details.error && (
        <div className="text-xs text-red-400 max-w-xs truncate" title={status.details.error}>
